document.querySelectorAll(".threejs-container").forEach((container) => {
  ...
  function loadModel(index) {
    ...
    loader.load(
      ...
      function (gltf) {
        ...
        // Make the camera look at the center (or any other point)
        camera.lookAt(0, 0, 0);
        ...
        currentModel.traverse((node) => {
          ...
          if (node.isMesh && elementSettings[node.name]) {
            ...
            if (elementSettings[node.name]) {
              ...
              node.visible =
                meshVisibility[node.name] !== undefined
                  ? meshVisibility[node.name]
                  : true;
            } else {
  ...
  for (let i = 1; i <= 2; i++) {
    // Loop from 1 to 2 for mesh1 and mesh2
    ...
    if (toggleButton) {
      toggleButton.addEventListener("click", () => {
        if (meshes[meshName]) {
          ...
          console.warn(
            `Toggle button clicked, but mesh ${meshName} does not exist.`,
          );
        }
      });
    }
